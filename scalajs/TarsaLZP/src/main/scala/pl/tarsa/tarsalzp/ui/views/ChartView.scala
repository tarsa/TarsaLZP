...
object ChartView {
  ...
  class D3Chart(svgWidth: Int, svgHeight: Int) {
    ...
    def drawPoints(element: TopNode, model: Props): Unit = {
      ...
      val totalChunks = totalSymbolsOpt.map(totalSymbols =>
        (totalSymbols + chunkSize - 1) / chunkSize).getOrElse(timeline.size)

      val svg = d3.select(element)
      val g = svg.select("g")

      val point = g.selectAll("rect")
        .data(timeline.toJsArray)

      point.enter().append("rect")

      def calculate[T](lens: Rect => T)
        (measurement: ChunkCodingMeasurement, index: Int): T = {
  ...
  class Backend(t: BackendScope[Props, Unit]) {
    ...
    def didMount(): Callback = {
      Callback {
        val element = t.getDOMNode()
        val model = t.props.runNow()
  ...
  private val component =
    ReactComponentB[Props]("ChartView")
      .stateless
      .renderBackend[Backend]
      .componentDidMount($ => $.backend.didMount())
      .componentDidUpdate($ => $.$.backend.didUpdate())
      .componentWillUnmount($ => $.backend.willUnmount())
      .configure(LogLifecycle.short)
      .build

  def apply(proxy: ModelProxy[Model]): ReactElement =
